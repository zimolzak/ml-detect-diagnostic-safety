if (OBJECT_ID('[Dflt].[SPADE_00_1_inputP]') is not null)	
	drop table [Dflt].SPADE_00_1_inputP
	
		CREATE TABLE [Dflt].SPADE_00_1_inputP(
		[sp_start] datetime2(0) NULL,
		[sp_end] datetime2(0) NULL)
	
INSERT INTO [Dflt].[SPADE_00_1_inputP]
           (
           [sp_start]
           ,[sp_end]
)
...
if (OBJECT_ID('[Dflt].SPADE_00_Gen_00_ICD10Code') is not null)
	drop table [Dflt].SPADE_00_Gen_00_ICD10Code

CREATE TABLE [Dflt].[SPADE_00_Gen_00_ICD10Code](
...
insert into [Dflt].[SPADE_00_Gen_00_ICD10Code](
	[CategoryName],
	[ICD10Code],
	[ICD10Description] 
)
...
if (OBJECT_ID('[Dflt].SPADE_00_Gen_00_ICD9Code') is not null)
	drop table [Dflt].SPADE_00_Gen_00_ICD9Code

CREATE TABLE [Dflt].[SPADE_00_Gen_00_ICD9Code](
	[CategoryName] [varchar](50) NULL,
...
insert into [Dflt].[SPADE_00_Gen_00_ICD9Code](
	[CategoryName],
	[ICD9Code],
	[ICD9Description] 
)
...
if (OBJECT_ID('[Dflt].[SPADE_010_01_InpatStrokeWithPrior30DayER]') is not null)	
	drop table [Dflt].SPADE_010_01_InpatStrokeWithPrior30DayER
go
...
select 
	...
	EDISLogSID
into  [dflt].[SPADE_010_01_InpatStrokeWithPrior30DayER] 
  FROM [Src].Inpat_InpatientDischargeDiagnosis a
  ...
  where AdmitDateTime between (select sp_start from [Dflt].SPADE_00_1_inputP)
						  and (select sp_end from [Dflt].SPADE_00_1_inputP)   
  and ( isnull(InpatIcd.ICD10Code,'') in              
			(select [ICD10Code] from [Dflt].[SPADE_00_Gen_00_ICD10Code] 
		     where [CategoryName]='Stroke')
	  or isnull(InpatIcd9.ICD9Code,'') in         													 
			(select [ICD9Code] from [Dflt].[SPADE_00_Gen_00_ICD9Code] 
		     where [CategoryName]='Stroke')     
	  )  
...
if (OBJECT_ID('[Dflt].[SPADE_020_02_InpatStrokeWithPrior30DayER_Outpat]') is not null)	
	drop table [Dflt].SPADE_020_02_InpatStrokeWithPrior30DayER_Outpat
go
...
into [dflt].[SPADE_020_02_InpatStrokeWithPrior30DayER_Outpat]
from [dflt].[SPADE_010_01_InpatStrokeWithPrior30DayER]  as a
...
if (OBJECT_ID('[Dflt].[SPADE_030_01_InpatStrokeWithPrior30DayER_Outpat_OnlyDizzHeadache]') is not null)	
	drop table [Dflt].SPADE_030_01_InpatStrokeWithPrior30DayER_Outpat_OnlyDizzHeadache
go

  select * 
  into [Dflt].[SPADE_030_01_InpatStrokeWithPrior30DayER_Outpat_OnlyDizzHeadache]  
  from [dflt].[SPADE_020_02_InpatStrokeWithPrior30DayER_Outpat] as a
  where (
  isnull(ERICD10code,'') in (select [ICD10Code] from [Dflt].[SPADE_00_Gen_00_ICD10Code]
					where [CategoryName] in ('Dizziness','Headache'))
or isnull(ERICD9code,'') in (select [ICD9Code] from [Dflt].[SPADE_00_Gen_00_ICD9Code]
					where [CategoryName] in ('Dizziness','Headache'))
			)
 and not exists(
 --from vDiagnosis
		select * from (
				...
				from  [dflt].[SPADE_020_02_InpatStrokeWithPrior30DayER_Outpat] as x
				where (isnull(ERICD10code,'') in (select [ICD10Code] from [Dflt].[SPADE_00_Gen_00_ICD10Code]
 ...
 --        select * from (
	...
	--		  where c.ICD10Code in (select [ICD10Code] from [Dflt].[SPADE_00_Gen_00_ICD10Code]
	--									where [CategoryName] in ('Stroke') )
	--	) as b
	--	where a.inpatientsid=b.inpatientsid and a.outpatVisitSID=b.outpatVisitSID
 )

if (OBJECT_ID('[Dflt].[SPADE_030_02_InpatStrokeWithPrior30DayE_Outpat_OnlyDizzHeadache_sentHome]') is not null)	
	drop table [Dflt].SPADE_030_02_InpatStrokeWithPrior30DayE_Outpat_OnlyDizzHeadache_sentHome
go
...
SELECT [inpatientsid]
	,b.PatientSSN
	  ...
	  ,vDiagnosisSID
	  ,ProblemListSID
into [dflt].[SPADE_030_02_InpatStrokeWithPrior30DayE_Outpat_OnlyDizzHeadache_sentHome]
from [dflt].[SPADE_030_01_InpatStrokeWithPrior30DayER_Outpat_OnlyDizzHeadache] as a
